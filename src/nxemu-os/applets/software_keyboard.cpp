...
void DefaultSoftwareKeyboardApplet::ShowTextCheckDialog(Service::AM::Frontend::SwkbdTextCheckResult text_check_result, std::u16string text_check_message) const
{
    LOG_WARNING(Service_AM, "(STUBBED) called, backend requested to show the text check dialog.");
}

void DefaultSoftwareKeyboardApplet::ShowInlineKeyboard(InlineAppearParameters appear_parameters) const
{
    LOG_WARNING(Service_AM, "(STUBBED) called, backend requested to show the inline software keyboard.");

    LOG_INFO(Service_AM, "\nInlineAppearParameters:\nmax_text_length={}\nmin_text_length={}\nkey_top_scale_x={}\nkey_top_scale_y={}\nkey_top_translate_x={}\nkey_top_translate_y={}\ntype={}\nkey_disable_flags={}\nkey_top_as_floating={}\nenable_backspace_button={}\nenable_return_button={}\ndisable_cancel_button={}", appear_parameters.max_text_length, appear_parameters.min_text_length, appear_parameters.key_top_scale_x, appear_parameters.key_top_scale_y, appear_parameters.key_top_translate_x, appear_parameters.key_top_translate_y, appear_parameters.type, appear_parameters.key_disable_flags.raw, appear_parameters.key_top_as_floating, appear_parameters.enable_backspace_button, appear_parameters.enable_return_button, appear_parameters.disable_cancel_button);
...
void DefaultSoftwareKeyboardApplet::InlineTextChanged(InlineTextParameters text_parameters) const
{
    ...
    LOG_INFO(Service_AM, "\nInlineTextParameters:\ninput_text={}\ncursor_position={}", Common::UTF16ToUTF8(text_parameters.input_text), text_parameters.cursor_position);

    submit_inline_callback(Service::AM::Frontend::SwkbdReplyType::ChangedString, text_parameters.input_text, text_parameters.cursor_position);
}
...
void DefaultSoftwareKeyboardApplet::SubmitInlineText(std::u16string_view text) const
{
    ...
    {
        submit_inline_callback(Service::AM::Frontend::SwkbdReplyType::ChangedString, std::u16string(text.data(), text.data() + index + 1), static_cast<s32>(index) + 1);
        std::this_thread::sleep_for(std::chrono::milliseconds(250));
    }

    submit_inline_callback(Service::AM::Frontend::SwkbdReplyType::DecidedEnter, std::u16string(text), static_cast<s32>(text.size()));
}