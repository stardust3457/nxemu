...
Result GetKeyCodeMapImpl(KeyCodeMap & out_key_code_map, KeyboardLayout keyboard_layout, LanguageCode language_code)
{
...
ISettingsServer::ISettingsServer(Core::System & system_) :
    ServiceFramework{system_, "set"}
{
    // clang-format off
...
Result ISettingsServer::GetAvailableLanguageCodes(Out<s32> out_count, OutArray<LanguageCode, BufferAttr_HipcPointer> out_language_codes)
{
    ...
    *out_count = static_cast<s32>(std::min(available_language_codes.size(), max_amount));
    ...
    memcpy(out_language_codes.data(), available_language_codes.data(), static_cast<std::size_t>(*out_count) * sizeof(LanguageCode));
...
Result ISettingsServer::GetAvailableLanguageCodes2(Out<s32> out_count, OutArray<LanguageCode, BufferAttr_HipcMapAlias> language_codes)
{
    LOG_DEBUG(Service_SET, "called");
    ...
    *out_count = static_cast<s32>(std::min(available_language_codes.size(), max_amount));
    ...
    memcpy(language_codes.data(), available_language_codes.data(), static_cast<std::size_t>(*out_count) * sizeof(LanguageCode));
    R_SUCCEED();
}
...
Result ISettingsServer::GetKeyCodeMap(OutLargeData<KeyCodeMap, BufferAttr_HipcMapAlias> out_key_code_map)
{
    LOG_DEBUG(Service_SET, "called");
...
Result ISettingsServer::GetKeyCodeMap2(OutLargeData<KeyCodeMap, BufferAttr_HipcMapAlias> out_key_code_map)
{
    LOG_DEBUG(Service_SET, "called");
...
Result ISettingsServer::GetDeviceNickName( OutLargeData<std::array<u8, 0x80>, BufferAttr_HipcMapAlias> out_device_name)
{
    LOG_DEBUG(Service_SET, "called");
