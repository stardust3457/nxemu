...
s64 CalendarTimeToEpoch(Service::PSC::Time::CalendarTime calendar)
{
    ...
    s8 month{static_cast<s8>(((month_s16 * 43) & ~std::numeric_limits<s16>::max()) + ((month_s16 * 43) >> 9))};
    ...
    auto epoch_s{((24ll * days_since_epoch + calendar.hour) * 60ll + calendar.minute) * 60ll + calendar.second};
...
Service::PSC::Time::LocationName GetTimeZoneString(Core::System & system, Service::PSC::Time::LocationName & in_name)
{
    ...
    ASSERT_MSG(IsTimeZoneBinaryValid(system, configured_name), "Invalid time zone {}!", configured_name.data());
...
TimeManager::TimeManager(Core::System & system) :
    m_steady_clock_resource{system}, 
    m_worker{system, m_steady_clock_resource, m_file_timestamp_worker},
    m_system(system)
{
    m_time_m = system.ServiceManager().GetService<Service::PSC::Time::ServiceManager>("time:m", true);
    ...
    m_set_sys->GetSettingsItemValueImpl<s32>(network_accuracy_m, "time", "standard_network_clock_sufficient_accuracy_minutes");
    auto one_minute_ns{ std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::minutes(1)).count()};
    ...
    res = m_set_sys->GetUserSystemClockAutomaticCorrectionUpdatedTime(&automatic_correction_time_point);
    ASSERT(res == ResultSuccess);

    res = m_time_m->SetupStandardUserSystemClockCore(is_automatic_correction_enabled, automatic_correction_time_point);
    ASSERT(res == ResultSuccess);
    ...
    if (m_file_timestamp_worker.m_system_clock->GetCurrentTime(&system_clock_time) == ResultSuccess)
    {
        Service::PSC::Time::CalendarTime calendar_time{};
        Service::PSC::Time::CalendarAdditionalInfo calendar_additional{};
        if (m_file_timestamp_worker.m_time_zone->ToCalendarTimeWithMyRule(&calendar_time, &calendar_additional, system_clock_time) == ResultSuccess)
        {
...
Result TimeManager::SetupStandardSteadyClockCore()
{
    ...
    m_set_sys->GetSettingsItemValueImpl<s32>(standard_steady_clock_test_offset_m, "time", "standard_steady_clock_test_offset_minutes");
    auto one_minute_ns{ std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::minutes(1)).count()};
    ...
    res = m_time_m->SetupStandardSteadyClockCore(reset_detected, clock_source_id, m_steady_clock_resource.GetTime(), external_steady_clock_internal_offset_ns, standard_steady_clock_test_offset_ns);
    ASSERT(res == ResultSuccess);
    R_SUCCEED();
}
...
Result TimeManager::SetupTimeZoneServiceCore()
{
    ...
    res = m_time_m->SetupTimeZoneServiceCore(name, rule_version, location_count, time_point, rule_buffer);
    ASSERT(res == ResultSuccess);
