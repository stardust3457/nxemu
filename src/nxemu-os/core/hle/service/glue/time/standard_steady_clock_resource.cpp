...
{
    out_time_s = std::chrono::duration_cast<std::chrono::seconds>(std::chrono::system_clock::now().time_since_epoch()).count();
...
StandardSteadyClockResource::StandardSteadyClockResource(Core::System & system) :
    m_system{system}
{
}

void StandardSteadyClockResource::Initialize(Common::UUID * out_source_id, Common::UUID * external_source_id)
{
    ...
    for (; i < NUM_TRIES; i++)
    {
        ...
        Kernel::Svc::SleepThread(m_system, std::chrono::duration_cast<std::chrono::nanoseconds>(std::chrono::milliseconds(1)).count());
    }
