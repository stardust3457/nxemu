...
IDirectory::IDirectory(Core::System & system_, IVirtualDirectoryPtr && directory_, FileSys::OpenDirectoryMode mode) :
    ServiceFramework{system_, "IDirectory"},
    backend(std::make_unique<FileSys::Fsa::IDirectory>(std::move(directory_), mode))
{
...
Result IDirectory::Read(Out<s64> out_count, const OutArray<FileSys::DirectoryEntry, BufferAttr_HipcMapAlias> out_entries)
{
