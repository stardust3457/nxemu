...
#include "yuzu_common/address_space.inc"
    ...
    cpu_backing_address(device_as_size >> DEVICE_PAGEBITS)
{
    ...
    const size_t total_virtual = device_as_size >> DEVICE_PAGEBITS;
...
void DeviceMemoryManager<Traits>::Map(DAddr address, VAddr virtual_address, size_t size, Asid asid, bool track)
{
    ...
    for (size_t i = 0; i < num_pages; i++)
    {
        ...
        auto phys_addr = static_cast<u32>(GetRawPhysicalAddr(ptr) >> DEVICE_PAGEBITS) + 1U;
...
void DeviceMemoryManager<Traits>::Unmap(DAddr address, size_t size)
{
    ...
    for (size_t i = 0; i < num_pages; i++)
    {
        ...
        if (phys_addr != 0) [[likely]]
        {
            ...
            const auto [more_entries, new_start] = impl->multi_dev_address.Unregister(static_cast<u32>(start_page_d + i), base_dev & MULTI_MASK);
            if (!more_entries)
            {
                compressed_device_addr[phys_addr - 1] = impl->multi_dev_address.ReleaseEntry(new_start);
                continue;
            }
...
void DeviceMemoryManager<Traits>::TrackContinuityImpl(DAddr address, VAddr virtual_address, size_t size, Asid asid)
{
    ...
    {
        size_t index = i - 1;
        const VAddr new_vaddress = virtual_address + index * DEVICE_PAGESIZE;
        const uintptr_t new_ptr = reinterpret_cast<uintptr_t>(process_memory->GetPointerSilent(new_vaddress));
...
template <typename Traits>
void DeviceMemoryManager<Traits>::InnerGatherDeviceAddresses(Common::ScratchBuffer<u32>& buffer, PAddr address)
{
...
{
    const size_t index = address >> DEVICE_PAGEBITS;
    const size_t offset = address & DEVICE_PAGEMASK;
    auto phys_addr = compressed_physical_ptr[index];
    if (phys_addr == 0) [[unlikely]]
    {
        return nullptr;
    }
    return GetPointerFromRaw<T>((static_cast<PAddr>(phys_addr - 1) << DEVICE_PAGEBITS) + offset);
}
...
void DeviceMemoryManager<Traits>::WalkBlock(DAddr addr, std::size_t size, auto on_unmapped, auto on_memory, auto increment)
{
    ...
    std::size_t page_index = addr >> DEVICE_PAGEBITS;
    std::size_t page_offset = addr & DEVICE_PAGEMASK;

    while (remaining_size)
    {
        const size_t next_pages = static_cast<std::size_t>(continuity_tracker[page_index]);
        const std::size_t copy_amount = std::min((next_pages << DEVICE_PAGEBITS) - page_offset, remaining_size);
        const auto current_vaddr = static_cast<u64>((page_index << DEVICE_PAGEBITS) + page_offset);
        SCOPE_EXIT{
        ...
        auto * mem_ptr = GetPointerFromRaw<u8>((static_cast<PAddr>(phys_addr - 1) << DEVICE_PAGEBITS) + page_offset);
...
void DeviceMemoryManager<Traits>::WriteBlock(DAddr address, const void* src_pointer, size_t size)
{
    WalkBlock(address, size, [&](size_t copy_amount, DAddr current_vaddr) 
        {
            LOG_ERROR(HW_Memory, "Unmapped Device WriteBlock @ 0x{:016X} (start address = 0x{:016X}, size = {})", current_vaddr, address, size);
        },
...
template <typename Traits>
void DeviceMemoryManager<Traits>::WriteBlockUnsafe(DAddr address, const void* src_pointer, size_t size)
{
    WalkBlock(address, size, [&](size_t copy_amount, DAddr current_vaddr)
        {
            LOG_ERROR(HW_Memory,"Unmapped Device WriteBlock @ 0x{:016X} (start address = 0x{:016X}, size = {})", current_vaddr, address, size);
...
void DeviceMemoryManager<Traits>::UpdatePagesCachedCount(DAddr addr, size_t size, s32 delta)
{
    ...
    const auto release_pending = [&]
    {
        if (uncache_bytes > 0)
        {
            ...
            {
                MarkRegionCaching(memory_device_inter, uncache_begin << DEVICE_PAGEBITS, uncache_bytes, false);
            }
            uncache_bytes = 0;
        }
        if (cache_bytes > 0)
        {
            if (memory_device_inter != nullptr)
            {
                MarkRegionCaching(memory_device_inter, cache_begin << DEVICE_PAGEBITS, cache_bytes, true);
            }
            cache_bytes = 0;
        }
    };
    size_t old_vpage = (base_vaddress >> DEVICE_PAGEBITS) - 1;
    for (; page != page_end; ++page)
    {
        CounterAtomicType& count = cached_pages->at(page >> subentries_shift).Count(page);
        auto [asid_2, vpage] = ExtractCPUBacking(page);
        vpage >>= DEVICE_PAGEBITS;
...
{
    static inline void MarkRegionCaching(IMemory * interface, VAddr address, size_t size, bool caching)
    {
