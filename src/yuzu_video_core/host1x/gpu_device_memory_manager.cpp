...
T * DeviceMemoryManager<Traits>::GetPointer(DAddr address)
{
    ...
    return GetPointerFromRaw<T>((static_cast<PAddr>(phys_addr - 1) << Memory::YUZU_PAGEBITS) + offset);
}
...
void DeviceMemoryManager<Traits>::WalkBlock(DAddr addr, std::size_t size, auto on_unmapped, auto on_memory, auto increment)
{
    ...
    while (remaining_size)
    {
        ...
        const auto current_vaddr = static_cast<u64>((page_index << Memory::YUZU_PAGEBITS) + page_offset);
        SCOPE_EXIT{
        ...
        auto * mem_ptr = GetPointerFromRaw<u8>((static_cast<PAddr>(phys_addr - 1) << Memory::YUZU_PAGEBITS) + page_offset);
...
void DeviceMemoryManager<Traits>::UpdatePagesCachedCount(DAddr addr, size_t size, s32 delta)
{
    ...
    const auto release_pending = [&]
    {
        if (uncache_bytes > 0)
        {
            ...
            {
                MarkRegionCaching(memory_device_inter, uncache_begin << Memory::YUZU_PAGEBITS, uncache_bytes, false);
            }
            uncache_bytes = 0;
        }
        if (cache_bytes > 0)
