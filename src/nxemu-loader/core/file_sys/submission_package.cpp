...
NSP::NSP(VirtualFile file_, uint64_t title_id_, std::size_t program_index_)
    ...
    if (pfs->GetStatus() != LoaderResultStatus::Success) {
...
LoaderResultStatus NSP::GetStatus() const {
    return status;
}

LoaderResultStatus NSP::GetProgramStatus() const {
...
uint64_t NSP::GetExtractedTitleID() const {
    ...
    if (meta.Load(GetExeFS()->GetFile("main.npdm")) == LoaderResultStatus::Success) {
...
void NSP::ReadNCAs(const std::vector<VirtualFile>& files) {
    for (const auto& outer_file : files) {
        ...
        for (const auto& inner_file : section0->GetFiles()) {
            ...
            ncas[cnmt.GetTitleID()][{cnmt.GetType(), LoaderContentRecordType::Meta}] = nca;
            ...
            for (const auto& rec : cnmt.GetContentRecords()) {
                ...
                if (next_file == nullptr) {
                    if (rec.type != LoaderContentRecordType::DeltaFragment) {
                ...
                if (next_nca->GetStatus() != LoaderResultStatus::Success &&
                    next_nca->GetStatus() != LoaderResultStatus::ErrorMissingBKTRBaseRomFS) {
                    continue;
                }
