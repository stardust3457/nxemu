...
LoaderResultStatus AppLoader_XCI::ReadUpdateRaw(FileSys::VirtualFile& out_file) {
    ...
    const auto read = xci->GetSecurePartitionNSP()->GetNCAFile(
        FileSys::GetUpdateTitleID(program_id), LoaderContentRecordType::Program);
    if (read == nullptr) {
...
LoaderResultStatus AppLoader_XCI::ReadIcon(std::vector<u8>& buffer) {
...
LoaderResultStatus AppLoader_XCI::ReadTitle(std::string& title) {
