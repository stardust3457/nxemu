...
AppLoader_DeconstructedRomDirectory::LoadResult AppLoader_DeconstructedRomDirectory::Load(Systemloader & loader, ISystemModules & systemModules)
{
    ...
    if (override_update) {
        const FileSys::PatchManager patch_manager(
            metadata.GetTitleID(), loader.GetFileSystemController(), loader.GetContentProvider());
    ...
    const bool is_39bit =
        metadata.GetAddressSpaceType() == ProgramAddressSpaceType::Is39Bit;
    ...
    const FileSys::PatchManager pm{metadata.GetTitleID(), loader.GetFileSystemController(),
    ...
    return {LoaderResultStatus::Success,
            LoadParameters{metadata.GetMainThreadPriority(), metadata.GetMainThreadStackSize(), base_address, processID}};
}
...
LoaderResultStatus AppLoader_DeconstructedRomDirectory::ReadIcon(std::vector<u8>& out_buffer) {
    if (icon_data.empty()) {
...
LoaderResultStatus AppLoader_DeconstructedRomDirectory::ReadTitle(std::string& out_title) {
    if (name.empty()) {
